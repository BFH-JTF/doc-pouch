...
export default class NeDbWrapper {
    ...
    async createDefaultStructures(): Promise<I_DataStructure[]> {
        const createdStructures: I_DataStructure[] = [];

        const dataImportStringStructure: I_DataStructure = {
            name: "Data Import String",
            description: "Template for data import",
            type: 0,
            subType: 2,
            fields: [
                {
                    name: "templateName",
                    displayName: "Name of Template",
                    type: "string",
                },
                {
                    name: "incomingStructureId",
                    displayName: "ID of incoming structure",
                    type: "string",
                },
                {
                    name: "importString",
                    displayName: "Import String",
                    type: "string",
                }
            ]
        };

        const existingDataImportStringResults = await this.structures.query({
            name: "Data Import String",
            type: 0,
            subType: 2
        }) as I_DataStructure[];
        let dataImportStringId: string | undefined;
        if (existingDataImportStringResults.length === 0) {
            const created = await this.structures.add(dataImportStringStructure) as I_DataStructure;
            this.logger.info(`Created new structure: ${JSON.stringify(created)}`);
            createdStructures.push(created);
            dataImportStringId = created._id;
        } else {
            dataImportStringId = existingDataImportStringResults[0]._id;
        }

        const dataImportTemplateStructure: I_DataStructure = {
            name: "Data Import Template",
            description: "This document describes what data can be imported from which structures.",
            type: 0,
            subType: 1,
            fields: [
                {
                    name: "toolID",
                    displayName: "ID of tool importing into",
                    type: "string",
                },
                {
                    name: "structures",
                    ...
                    type: "array",
                    items: dataImportStringId || "string",
                }
            ]
        };

        const existingDataImportTemplateResults = await this.structures.query({
            name: "Data Import Template",
            type: 0,
            subType: 1
        }) as I_DataStructure[];
        if (existingDataImportTemplateResults.length === 0) {
            const created = await this.structures.add(dataImportTemplateStructure) as I_DataStructure;
            this.logger.info(`Created new structure: ${JSON.stringify(created)}`);
            createdStructures.push(created);
        }

        const toolDefinitionStructure: I_DataStructure = {
        ...
        const existingToolDefinitionResults = await this.structures.query({
            name: "Tool Definition",
            type: 0,
            subType: 0
        }) as I_DataStructure[];
        if (existingToolDefinitionResults.length === 0) {
            const created = await this.structures.add(toolDefinitionStructure) as I_DataStructure;
            this.logger.info(`Created new structure: ${JSON.stringify(created)}`);
            createdStructures.push(created);
        }
